...
**MarkLuck** — 轻量化、本地优先、离线可用的 Markdown 笔记工具。每一条笔记就是一个纯文本笔记文件（应用内支持 `.md` / `.markdown` / `.mdx` / `.txt`），文件夹即笔记本。数据完全由用户掌控。

**核心闭环**：

```
用户打开本地文件夹 → 浏览/编辑支持格式笔记文件 → 实时 Markdown 渲染预览
...
| 导出表格      | **write-excel-file**        | 纯 JS，替代存在高危审计问题的 SheetJS `xlsx` 运行时依赖     |
...
```
你是 MarkLuck 笔记应用的视觉回归测试专家。请分析这张截图 (检查点: {name})。
...
## 六、设计系统 — 纸张隐喻 (Paper Metaphor)

> ⚠️ 2026-06-04 重构：原"构成主义+磨玻璃"双主题体系已替换为单一纸张隐喻主题。
> `spec/frontend/design-system.md` 使用旧 hex 色值体系，**实际权威来源是以下代码文件**：
...
**隐喻**：日本和纸 + iA Writer 专注感。暖调米白底，墨色文字，单一冷调强调色。
UI 退后，内容浮现。
...
│   ├── renderer/                # Markdown 渲染库 (@markluck/renderer)
│   │   └── src/
