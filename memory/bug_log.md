...
## BUG-129: 公共模型停止后仍有多份训练说明和可执行式旧命令

- **现象**: V2R/V2S 已被 architecture stop 阻断，但语料 README、V2 计划和补全规格仍分别维护训练步骤；语料 README 还把旧 v4 学习曲线和 publisher 命令写成普通操作，维护者难以区分历史复核、停止态检查和未来新架构流程。
- **根因**: 公共模型经历 v4、V2R、V2S 三轮演进时持续追加文档，没有指定唯一训练操作说明；架构停止只进入代码和证据记录，未同步收口文档入口。
- **根因类别**: 补全模型 / 文档一致性 / 生命周期 / 发布证据
- **修复**: 新增 `doc/autocomplete-model-training.md` 作为唯一训练操作说明，集中记录现状、历史结果、语料治理、模型参数、评测分母、证据链、停止态命令和未来重启条件；`scripts/corpus/README.md` 精简为目录索引，V2 计划与补全规格只保留历史结论和权威链接。
- **教训**: architecture stop 不只要阻断代码入口，还要收口人类操作入口；历史命令必须明确标成不可继续执行，避免文档重新开启已停止路线。

## 检查清单追加

- [ ] 公共模型停止后是否只保留一份权威训练操作说明，其他文档仅链接而不复制命令？
- [ ] 训练文档是否明确区分只读复核、历史实验和未来新架构流程，并把 RC code 10 说明为安全结果？

## BUG-130: 外部单文件 grant 被错误扩大为父目录权限

- **现象**: 通过文件关联打开一个 Markdown 后，同一 token 可以读取、写入、枚举和监听同级其他文件。
- **根因**: `ExternalAccessGrant` 只保存父目录 root，`resolve_file()` 未绑定获授权的目标文件；opened-file capabilities 默认开放 list/watch。
- **根因类别**: 文件IO / 权限边界
- **修复**: grant 同时绑定 canonical 文件；默认仅允许该文件读写，拒绝同级访问与目录能力；只有 `open_external_notebook` 显式用户动作才提升为目录 grant。
- **教训**: opaque token 只是凭据形式，权限范围仍必须绑定到用户实际选择的对象。

## BUG-131: 异步会话与索引缺少 generation 导致旧结果回写

- **现象**: 快速打开外部文件 A/B 或切换笔记本时，较慢的旧任务可能覆盖当前内容、标签和搜索状态。
- **根因**: `enterExternalFileSession()` 与 `indexStore.initialize(force=true)` 在 await 后未验证任务是否仍属于当前会话。
- **根因类别**: 状态管理 / 文件IO
- **修复**: 外部会话与索引初始化分别引入单调 generation；过期结果不提交状态，并撤销过期外部 token。
- **教训**: 所有跨文件、跨根目录的异步读取都必须在提交状态前核对会话身份。

## BUG-132: MockFS 与 Tauri 的路径和重命名语义不一致

- **现象**: `C:/Notes2` 被误判为 `C:/Notes` 内部路径；MockFS 重命名会覆盖已存在目标，而桌面端行为依平台变化。
- **根因**: 路径归属使用字符串 `startsWith`，MockFS 则恒返回 true；重命名未显式定义 collision 策略。
- **根因类别**: 文件IO / 跨平台兼容
- **修复**: 使用规范化路径段边界判断；Mock 与 Rust 均在目标已存在时 fail-closed，保留源和目标内容。
- **教训**: 文件系统抽象必须统一安全边界与失败语义，不能让测试替身比真实后端更宽松。

## BUG-133: 文件 watcher 后端存在但工作区从未订阅

- **现象**: 外部修改、移动或删除笔记后，文件树与索引不刷新，脏编辑内容还可能覆盖外部变化。
- **根因**: `TauriIPCService.watch()` 已实现，但 `NotebookHome` 没有建立或清理 watcher 生命周期。
- **根因类别**: 文件IO / 状态管理
- **修复**: 笔记本切换时以 generation 替换 watcher；事件合并后刷新树与索引，活动文件外部变化执行冲突保护；卸载和草稿模式停止 watcher。
- **教训**: 后端能力存在不代表产品链路接通，必须从事件源追到最终 UI 状态。

## BUG-134: 外部只读预览被迫加载完整编辑器依赖

- **现象**: 通过 Markdown 文件启动时，即使只读预览，也要先下载和解析 CodeMirror 及多个非首屏界面。
- **根因**: `NotebookHome.vue` 静态导入 `MarkdownEditor`、主题中心、文件抽屉和非首屏浮层。
- **根因类别**: 性能 / 启动链路
- **修复**: 重组件改为异步组件；外部只读路径在用户点击“启用编辑”前不加载 MarkdownEditor/CodeMirror；补全训练延迟到首屏之后，并记录 shell/editor ready 性能标记。
- **教训**: 文件关联启动必须是独立的最小读取路径，不能继承完整工作区的依赖闭包。
