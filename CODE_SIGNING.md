...
MarkLuck 不收集、不存储、不上传任何用户数据。所有笔记数据完全存储于用户本地设备。
...
MarkLuck 使用可重现构建（reproducible builds）。所有签名产物均可通过以下方式验证：

```bash
# 验证签名
signtool verify /pa /v MarkLuck.msi
