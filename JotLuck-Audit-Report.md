...
| L7  | `MarkdownPredictor.ts:304-319` | `predictFormatClosure` 硬编码了闭合文本 "粗体\*_", "斜体_", "code`", "强调\_\_"     | 闭合文本应从上下文推断而非硬编码，目前仅 `**` 场景工作良好 |
...
| 预测器在 localStorage 中持久化统计 | ✅ PASS | `jotluck:ngram:v2` 或 meta 存在               |
...
| 键                             | 类型   |      大小范围      |                    验证结果                    |
| ------------------------------ | ------ | :----------------: | :--------------------------------------------: |
| `jotluck:ngram:v2`             | string |     0 - 3.5MB      |                 ✅ 跨重载存活                  |
| `jotluck:ngram:meta`           | JSON   |       ~100B        | ✅ `{v:2, docs:N, totalEntries:N, lastSave:T}` |
| `jotluck:autocomplete:enabled` | string | `"true"`/`"false"` |              ✅ 设置开关正确写入               |

**验证方法**：

- COLD START: `page.evaluate(() => localStorage.clear())` → 训练 5 次 → 确认 `jotluck:ngram:v2` 非 null → `closeDocument()` 触发合并
- CROSS-SESSION: 记录重载前 L2 `ngramDataSize` → `page.reload()` → 确认重载后 `ngramDataSize > 0`
- SETTINGS TOGGLE: 确认 `jotluck:autocomplete:enabled` 从非 `'false'` → `'false'` → 非 `'false'` 的完整切换周期
