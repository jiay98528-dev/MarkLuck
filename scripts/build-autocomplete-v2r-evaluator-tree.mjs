...
export function buildAutocompleteV2REvaluatorTree({ rootDir = REPOSITORY_ROOT, outputPath }) {
  const root = realpathSync(path.resolve(rootDir));
  const output = resolveWorkspaceOutput(root, outputPath);
  if (existsSync(output)) {
    throw new Error('V2R evaluator source-tree evidence already exists and cannot be overwritten.');
  }
  const files = AUTOCOMPLETE_V2R_EVIDENCE_SOURCE_FILES.map((relativePath) => ({
    path: relativePath,
...
function resolveWorkspaceOutput(rootDir, value) {
  if (!value || path.isAbsolute(value)) {
    throw new Error('V2R evaluator output must be a repository-relative path.');
  }
  const resolved = path.resolve(rootDir, value);
  const relative = path.relative(rootDir, resolved);
  if (relative.startsWith('..') || path.isAbsolute(relative)) {
    throw new Error('V2R evaluator output escaped the repository.');
  }
  let existing = path.dirname(resolved);
  while (!existsSync(existing)) {
    const parent = path.dirname(existing);
    if (parent === existing) throw new Error('V2R evaluator output has no safe parent.');
    existing = parent;
  }
  const realRelative = path.relative(rootDir, realpathSync(existing));
  if (realRelative.startsWith('..') || path.isAbsolute(realRelative)) {
    throw new Error('V2R evaluator output escaped through a symlink or junction.');
  }
  return resolved;
}
...
function parseArguments(argv) {
  ...
  for (const argument of argv) {
    ...
    else throw new Error(`Unknown V2R evaluator-tree argument: ${argument}`);
  }
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  try {
    console.log(
      JSON.stringify(
        buildAutocompleteV2REvaluatorTree(parseArguments(process.argv.slice(2))),
        null,
        2,
      ),
    );
